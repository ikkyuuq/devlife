...
export const EditTask = () => {
  ...
  useEffect(() => {
    const fetchTask = async () => {
      ...
      if (!resp.ok) {
        console.error("Error fetching task:", resp.statusText);
        return;
      }
      ...
      setValue("objective", data.task.objective);
      setValue("content", data.task.content);
      setValue("tags", data.task.tags);
      setValue("tests", JSON.parse(data.task.tests).data);
    };
  ...
  const onSubmit = async (data: TaskFormValues) => {
    const taskId = id || data.title.toLowerCase().replace(/\s+/g, "");
